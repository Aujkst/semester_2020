...
```{r boston splines}
# 先使用50%分位点
fit_splines <- lm(nox ~ bs(dis, df = 4))
pre_splines <- predict(fit_splines, newdata = list(dis = dis_grid), se = TRUE)
plot(dis, nox, col = "#a9a9a9a1")
lines(dis_grid, pre_splines$fit, lwd = 2)
lines(dis_grid, pre_splines$fit + 2 * pre_splines$se, lty = "dashed")
lines(dis_grid, pre_splines$fit - 2 * pre_splines$se, lty = "dashed")
